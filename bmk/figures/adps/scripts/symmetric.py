...
starting_adps = ADP(atoms, adps=adp_tensor * 1.5)
...
# view(target_atoms)
# view(traj)
plot_pdf(s.get_r(), target_pdf, s.get_pdf(traj[np.argmin(nrg)]), show=False, save_file='../symmetric/min')
...
plt.savefig(save_file + '_mag_dist.eps', bbox_inches='tight',
            transparent='True')
plt.savefig(save_file + '_mag_dist.png', bbox_inches='tight',
            transparent='True')
plt.show()
