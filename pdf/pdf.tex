...
\foreach \n/\o in {no_mask/with no mask, edge_mask/with only an edge mask, auto_mask/combining an edge mask combined and the automaticly generated mask}{
    ...
    \caption{Masking, average, and \% standard deviation of an example x-ray total scattering measurement. This image was produced \o. a) the image, b) the mean and median values, c) \% standard deviation (normalized to the median), d) a closeup of the 28 \AA$^{-1}$ to 31 \AA$^{-1}$ $Q$ range for the mean and median, e) \AA$^{-1}$ to 31 \AA$^{-1}$ $Q$ range for the standard deviation}
    \end{figure}
}
\end{landscape}